...
function RTCPeerConnection() {
  ...
  var signalingClient=arguments[1];
  var remoteClientId=arguments[2];
  ...
  pc.onconnectionstatechange = function onconnectionstatechange() {
    self.dispatchEvent({ type: 'connectionstatechange', 'state':this.connectionState});
  ...
  pc.oniceconnectionstatechange = function oniceconnectionstatechange() {
    self.dispatchEvent({ type: 'iceconnectionstatechange','iceConnectionState':self.iceConnectionState});
...
RTCPeerConnection.prototype.onicecandidate = function onicecandidate(candidate) {
  if (candidate.candidate !=null)
    this.signalingClient.sendIceCandidate(candidate.candidate, this.remoteClientId);
